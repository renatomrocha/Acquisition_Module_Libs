...
N_MAX31856::N_MAX31856(int8_t spi_cs, int8_t spi_mosi, int8_t spi_miso, int8_t spi_clk) {
  _sclk = spi_clk;
  _cs = spi_cs;
  _miso = spi_miso;
  _mosi = spi_mosi;
...
// Hardware SPI init
N_MAX31856::N_MAX31856(int8_t spi_cs) {
  _cs = spi_cs;
  _sclk = _miso = _mosi = -1;
}

boolean N_MAX31856::begin(void) {
  pinMode(9, OUTPUT);
  pinMode(10,OUTPUT);
  pinMode(11, OUTPUT);
  pinMode(12,OUTPUT);
...
void N_MAX31856::setThermocoupleType(max31856_thermocoupletype_t type) {
...
max31856_thermocoupletype_t N_MAX31856::getThermocoupleType(void) {
  uint8_t t = readRegister8(MAX31856_CR1_REG);
  t &= 0x0F;
...
uint8_t N_MAX31856::readFault(void) {
  return readRegister8(MAX31856_SR_REG);
}

void N_MAX31856::setColdJunctionFaultThreshholds(int8_t low, int8_t high) {
  writeRegister8(MAX31856_CJLF_REG, low);
  writeRegister8(MAX31856_CJHF_REG, high);
}

void N_MAX31856::setTempFaultThreshholds(float flow, float fhigh) {
  int16_t low, high;
...
void N_MAX31856::oneShotTemperature(void) {
  ...
  delay(100); // MEME FIX autocalculate based on oversampling
}



float N_MAX31856::readCJTemperature(void) {
  oneShotTemperature();
...
float N_MAX31856::readThermocoupleTemperature(void) {
  oneShotTemperature();
...
uint8_t N_MAX31856::readRegister8(uint8_t addr) {
  uint8_t ret = 0;
...
uint16_t N_MAX31856::readRegister16(uint8_t addr) {
...
uint32_t N_MAX31856::readRegister24(uint8_t addr) {
...
void N_MAX31856::readRegisterN(uint8_t addr, uint8_t buffer[], uint8_t n) {
...
void N_MAX31856::writeRegister8(uint8_t addr, uint8_t data) {
...
uint8_t N_MAX31856::spixfer(uint8_t x) {
  if (_sclk == -1)
...
void N_MAX31856::selectSlot(int8_t slot){
switch(slot){
